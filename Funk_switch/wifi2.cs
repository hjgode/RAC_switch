...
namespace RAC_switch
{
    ...
    {
        itc_ssapi2 _ssAPI = new itc_ssapi2();
        ...
        public wifi()
        {
            if (!isFUNKclient())
            {
                throw new NotSupportedException("No Funk client installation");
            }
        ...
        bool isFUNKclient()
        {
            return RAC_switch.itc_ssapi2.isFunkClient();
        }
        ...
        void start()
        {
            ...
            foreach (itc_ssapi2.funkProfile R in _ssAPI._funkProfiles)
            {
        ...
        public int setFunk(string sProfile)
        {
            return _ssAPI.setFUNKprofile(sProfile);
        }

        public List<itc_ssapi2.funkProfile> funkProfiles
        {
            get
            {
                return _ssAPI._funkProfiles;
            }
        }
                    ...
                    foreach (AccessPoint ap in adapter.NearbyAccessPoints)
                    {
                        ...
                        foreach (string s in preferedSSIDs)
                        {
                            if (ap.Name == s) // && adapter.AssociatedAccessPoint != s) 
                            {
                                ...
                                _ssAPI.setFUNKprofileBySSID(s);//switch
                            }
